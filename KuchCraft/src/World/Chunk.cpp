...
namespace KuchCraft {
			...
			for (int y = 0; y < chunk_size_Y; y++)
			{
				for (int z = 0; z < chunk_size_XZ; z++)
				{
					...
					if (!checkLeft  && leftChunk)
						AddToDrawList({ x, y, z }, leftChunk->Items[chunk_size_XZ - 1][y][z],   vertices_left_index);
					if (!checkRight && rightChunk)											    					   
						AddToDrawList({ x, y, z }, rightChunk->Items[0][y][z],                  vertices_right_index);

					if (!checkBehind && behindChunk)
						AddToDrawList({ x, y, z }, behindChunk->Items[x][y][chunk_size_XZ - 1], vertices_behind_index);
					if (!checkFront  && frontChunk)
						AddToDrawList({ x, y, z }, frontChunk->Items[x][y][0],                  vertices_front_index);

					if (!checkTop)
						AddToDrawList({ x, y, z }, Item(ItemType::Air), vertices_top_index);

					if (checkBottom)
						AddToDrawList({ x, y, z }, Items[x][y - 1][z], vertices_bottom_index);				
					if (checkTop)
						AddToDrawList({ x, y, z }, Items[x][y + 1][z], vertices_top_index);
				
					if (checkBehind)
						AddToDrawList({ x, y, z }, Items[x][y][z - 1], vertices_behind_index);			
					if (checkFront)
						AddToDrawList({ x, y, z }, Items[x][y][z + 1], vertices_front_index);

					if (checkLeft)
						AddToDrawList({ x, y, z }, Items[x - 1][y][z], vertices_left_index);
					if (checkRight)
						AddToDrawList({ x, y, z }, Items[x + 1][y][z], vertices_right_index);			
				}
			}
		}			
	...
	void Chunk::AddToDrawList(const glm::ivec3& position, Item neighbourItem, uint32_t verticesIndex)
	{
		Item item = Items[position.x][position.y][position.z];

		if (item.IsTransparentBlock())
		{
			if (item.IsTransparentCumulatively())
			{
				if (item.Type != neighbourItem.Type && !neighbourItem.IsSolidBlock())
					m_DrawList.AddTransparentQuad(m_Position + glm::vec3{ position }, item, verticesIndex);
			}
			else
				m_DrawList.AddTransparent(m_Position + glm::vec3{ position }, item);		
		}
		else if (item.IsSolidBlock() && neighbourItem.IsTranslucent())
			m_DrawList.AddSolid(position, verticesIndex, item);	
	}
